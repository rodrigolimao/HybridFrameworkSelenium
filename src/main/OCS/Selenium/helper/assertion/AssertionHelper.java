...
public class AssertionHelper {
    ...
    public static void verifyText(String s1, String s2) {
        log.info("Checking text: " + s1 + " with " + s2);
        Assert.assertEquals(s1, s1);
    }

    public static void markPass() {
        log.info("Making script to PASS..");
        Assert.assertTrue(true);
    }

    public static void markPass(String message) {
        log.info("Making script to PASS.." + message);
        Assert.assertTrue(true, message);
    }

    public static void markFail() {
        log.info("Making script to FAIL..");
        Assert.assertTrue(false);
    }

    public static void markFail(String message) {
        log.info("Making script to FAIL.." + message);
